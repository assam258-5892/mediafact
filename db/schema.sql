...
CREATE TABLE IF NOT EXISTS 기사 (
    ...
    공개여부 INTEGER DEFAULT 0, -- 0: 비공개, 1: 공개
...
CREATE VIRTUAL TABLE IF NOT EXISTS 기사전문색인 USING fts5(
    기사번호 UNINDEXED, -- 기사 고유번호(색인용, 검색대상 아님)
    기사제목,           -- 기사 제목
    기사부제,           -- 기사 부제
    기사요약,           -- 기사 요약
    기사내용,           -- 기사 본문
...
-- 사진 및 기사 정보 전문검색을 위한 FTS5 색인 테이블
-- 사진설명, 기사제목, 기사부제, 기사요약, 기사내용 컬럼을 포함
CREATE VIRTUAL TABLE IF NOT EXISTS 사진전문색인 USING fts5(
    사진연번 UNINDEXED, -- 사진 고유번호(색인용, 검색대상 아님)
    사진설명,           -- 사진 설명(검색/관리용)
    기사제목,           -- 사진이 속한 기사 제목
    기사부제,           -- 사진이 속한 기사 부제
    기사요약,           -- 사진이 속한 기사 요약
